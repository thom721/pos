...
"@
    ...
    if (-not (Test-Path $BundledDataDir)) {
      ...
      # Prendre la propriete + ACL explicites (/grant:r evite les ACL refus heritees)
      try {
        takeown /F "$BundledDataDir" /D Y 2>&1 | Out-Null
        icacls "$BundledDataDir" /inheritance:r `
...
"@
    ...
    if (-not (Test-Path $dataDir)) {
      ...
      try {
        takeown /F "$dataDir" /D Y 2>&1 | Out-Null
        icacls "$dataDir" /inheritance:r /grant:r "SYSTEM:(OI)(CI)F" /grant:r "Administrators:(OI)(CI)F" /grant:r "Users:(OI)(CI)F" | Out-Null
      } catch {}
