...
@router.post("/categories/", response_model=Union[CategoryRead, List[CategoryRead]])
def create_category(
    data: Union[CategoryCreate, List[CategoryCreate]],
    background_tasks: BackgroundTasks,
    db: Session = Depends(get_db),
    current_user: User = Depends(require_permission(P.CATEGORIES_CREATE)),
):
    result = CategoryService(db, tenant_id=current_user.tenant_id).create(data)
