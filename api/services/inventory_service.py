...
def _stock_map(db: Session, product_ids: list[str]) -> dict[str, float]:
...
def get_preview(db: Session, category_ids: list[str] | None = None) -> list[dict]:
...
def list_inventories(db: Session, page: int = 1, limit: int = 20) -> dict:
...
def get_inventory(db: Session, inventory_id: str) -> InventoryRecord | None:
    return db.get(InventoryRecord, inventory_id)


def create_inventory(db: Session, data, user_id: str) -> InventoryRecord:
    if not data.items:
        raise HTTPException(400, "Aucun produit compté")

    product_ids = [str(item.product_id) for item in data.items]
    stocks = _stock_map(db, product_ids)
